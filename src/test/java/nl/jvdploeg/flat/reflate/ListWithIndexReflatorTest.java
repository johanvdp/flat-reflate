...
public final class ListWithIndexReflatorTest {
  ...
  public void testDeflate() throws Exception {
    ...
    // when
    reflator.deflate(object, field, model, new Path("test", "[index]"));
    // then
  ...
  public void testInflate() throws Exception {
    ...
    // when
    reflator.inflate(model, new Path("test", "[index]"), object, field);
    // then
    ...
    Assert.assertEquals("0", object.getValue().get(0).getIndex());
    Assert.assertEquals("1", object.getValue().get(1).getIndex());
  }
